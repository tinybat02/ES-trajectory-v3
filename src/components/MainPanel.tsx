    ...
    if (prevState.current !== this.state.current) {
      ...
      if (this.state.current !== 'None') {
        ...
        if (routeData.length > 1) {
          const firstLine = createLineWithLabel(routeData, timeData, 0, floorData);
          partialRoute.push(firstLine);
          for (let i = 0; i < routeData.length - 1; i++) {
            totalRoute.push(createLine(routeData, i, floorData));
          }
        }

        const totalPoints: Feature<Point>[] = [];
        for (let i = 0; i < routeData.length; i++) {
          totalPoints.push(createPoint(routeData, uncertaintyData, i, floorData));
        }
        ...
        const firstPoint = createPoint(routeData, uncertaintyData, 0, floorData);
        pointFeatures.push(firstPoint);
        if (routeData.length > 1) {
          const secondPoint = createPoint(routeData, uncertaintyData, 1, floorData);
  ...
  handleIterRoute = (type: string) => () => {
    ...
    if (type === 'previous' && iterRoute > 0) {
      ...
      this.setState({ iterRoute: iterRoute - 1 }, () => {
        const lineFeature = createLineWithLabel(routeData, timeData, this.state.iterRoute, floorData);
        const beginPoint = createPoint(routeData, uncertaintyData, this.state.iterRoute, floorData);
        const endPoint = createPoint(routeData, uncertaintyData, this.state.iterRoute + 1, floorData);
